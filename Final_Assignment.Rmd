...
logAGB_temp <- ggplot(data = temperate_Data, aes(x = logBGB, y = logAGB))  +
  ...
  # dd the equation label to the graph, two seperate lines, one for eq and for rr
...
Fig6 <- ggplot(data = fDataGood, aes(x = Species.Richness, y = AGB)) +
  # Set separate aestehtic for geom_point so that when linear model is taken
  ...
  # Set the legend position and remove the title
